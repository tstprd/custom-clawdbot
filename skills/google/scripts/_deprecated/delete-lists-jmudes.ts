/**
 * Delete Bébé and Travaux Maison lists from jmudes account
 */
...
async function main(): Promise<void> {
  try {
    ...
    let result = '🗑️ Suppression des listes (jmudes76000@gmail.com):\n';
    
    const listsToDelete = ['Bébé', 'Travaux Maison'];
    
    for (const list of listsData.items || []) {
      if (listsToDelete.includes(list.title)) {
        await apiRequest(
          `https://tasks.googleapis.com/tasks/v1/users/@me/lists/${list.id}`,
          { method: 'DELETE' }
        );
        result += `\n✅ Supprimée: ${list.title}`;
      }
    }
