...
try {
    ...
    $tokenData | ConvertTo-Json -Depth 10 | Out-File -FilePath ".clawdbot-google-tokens.json" -Encoding UTF8
    
    $result = "✅ Authentification réussie pour alejmurot@gmail.com!`n`nTokens sauvegardés dans .clawdbot-google-tokens.json`n`nScopes: $($response.scope)"
    Write-Output $result
