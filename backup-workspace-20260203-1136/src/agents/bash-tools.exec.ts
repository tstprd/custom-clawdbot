...
export function createExecTool(
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      if (process.platform === "win32" && !baseEnv.PYTHONIOENCODING) {
      ...
      const transformedCommand = transformCommandForPowerShell(params.command);
      const child = sandbox
        ? spawn(
            "docker",
            buildDockerExecArgs({
        ...
        : spawn(shell, [...shellArgs, transformedCommand], {
            cwd: workdir,
            env,
            detached: process.platform !== "win32",
            stdio: ["pipe", "pipe", "pipe"],
